...
double x1 = 2.0; double x2 = 2.201; double x3 = 3.28; double x5 = 0.3;//用来作为除数的参数
double x4 = 1.6;//用来作为乘数的参数，有风险的参数，应该试一下1
double idle_rate(){ return 1.0*total_idle_sev / total_sev; }//服务器闲置率∈(0,1)
void strcpy1(char model[], char *&m)//read_mode1 
{
	int len = strlen(m);
	for (int i = 0; i < len; i++)
	{
...
void strcpy2(char model[], char m[])//read_mode2
{
	int len = strlen(m);
	for (int i = 0; i < len; i++)
	{
...
int strcmp1(char opt[], char s[])//cmp_typename
{
	int len = strlen(s);
	for (int i = 0; i < len; i++)
	{
...
struct Sever
{
	...
	double evaluate(int left_day)//cost_value_par1
	{
		//double rate1 = double(CPU_core_demand) / instorage_demand;
		//double rate2 = double(core) / memory;
		//return (hardcost + dailycost*left_day) / (core + memory - abs(memory - core) / x1);//目前最优的购买策略，2.25是调出来的，与具体数据集有关
		return (hardcost + dailycost*left_day)*T*1.0 / (core + memory - abs(memory - core) / x1);
		//if (left_day>T*0.9)
		//	return (hardcost + dailycost*left_day)*T*1.0/ (core + memory - abs(memory - core) / x1)-(core+memory)*16.0;
		//else 
		//	return (hardcost + dailycost*left_day)*T*1.0 / (core + memory - abs(memory - core) / x1) + (hardcost + dailycost*left_day);
		//return (hardcost + dailycost*left_day) / (core + memory - abs(memory - core) / 2.35 + 1 * fabs(rate1 - rate2));
		//return ((hardcost + dailycost*left_day) / (core + memory - abs(memory - core) / 2*(double(T*2)/double(T+left_day))));
		//return ((hardcost + dailycost*left_day) / (core + memory - abs(memory - core) / 2.5));
		//return((hardcost + dailycost*left_day) / (core + memory));
		//return 1000000/(core + memory);
	}
...
struct Vmachine
{
	...
	int core;
	int memory;
	int isbinode;//0 unnode，1 binode（0表示单节点部署，1表示双节点
...
struct Node
{
	...
	Vma vma[1025]; int number_vma = 0;//用数组还是别的数据结构存放一个节点负载的虚拟机//can be skipped
...
struct Opt_list//at most 2e5+7days
{
	int top = 0;
...
struct Vma_record//之前想把部署的虚拟机一个个按大小顺序连成链表（挺耗时），用于迁移，后来这种迁移策略弃用了
{
	vma_node*head = NULL;
...
map<int, int>mp2;//从模拟服务器编号+1映射到真服务器编号+1，【优化的时候是不用管mp2的】
void del_vma(int vma_id)//根据虚拟机id进行删除
{
	int sevid = mp[vma_id] - 1;
	mp[vma_id] = 0; total_vma--;
	//vma_record.del(vma_id);
...
int add_vma(int vma_type, int vma_id)//把虚拟机部署到最合适的服务器上并记录，返回0表示装不下了，得买；返回1表示成功部署
{
	...
	double x = x2;//参数，初赛的练习赛和正式赛最优好像都是x==2.0
	//double rate0 = 0.02;
	if (vmachine[vma_type].isbinode == 0)//单节点部署
	{
		...
		for (int i = 0; i < total_sev; i++)
		{
			...
			{
				if (left_core + left_memory + abs(delta_sev - delta_vma) / x < all)//是否这样选出的服务器最合适(可能要考虑core与memory的比例或者差值)【可能多方面，考虑日费？
				{
					//min_core_left = left_core;
					//min_memory_left = left_memory;
		...
		//cout << "(" << min_left_sev_id << ", " << (min_left_sev_node == 0 ? "A" : "B") << ")\n";
		add_list.sev_id[add_list.p] = min_left_sev_id; add_list.sev_node[add_list.p] = min_left_sev_node; add_list.p++;
		total_vma++;// vma_record.add(vma_id, vma_type,min_left_sev_node); 
		///vma_record0.add(vma_id, vma_type, min_left_sev_node);
		return 1;
	}
	else if (vmachine[vma_type].isbinode == 1)
	{
		...
		for (int i = 0; i < total_sev; i++)
		{
			...
			if (left_core0 >= need_core&&left_core1 >= need_core&&left_memory0 >= need_memory&&left_memory1 >= need_memory)
			{
				...
				{
					//min_core_left0 = left_core0;
					//min_core_left1= left_core1;
					//min_memory_left = left_memory0;
					//min_memory_left1= left_memory1;
		...
		//cout << "(" << min_left_sev_id << ")\n";
		add_list.sev_id[add_list.p] = min_left_sev_id; add_list.sev_node[add_list.p] = 2; add_list.p++;
		total_vma++;
		//vma_record.add(vma_id, vma_type,2); vma_record1.add(vma_id, vma_type,2);
		return 1;
	}
}
...
void move4()
{
	for (int i = 0; i < total_sev; i++)
	{
		vis_sev[i] = 0;//迁入的不能再迁出，迁出的【可以】再迁入
		target_sev[i].clear();
	}
	...
	double x = x3;//参数
	double y = x4;
	for (daily_migration.W = 0; daily_migration.W < 3 * total_vma / 100;)
	{
...
void move7()
{
	...
	double x = x3;//参数
	double y = x4;
	for (daily_migration.W = 0; daily_migration.W < total_vma;)
	{
		min_used_core = 1e8; min_used_memory = 1e8; int flag = 0;
		for (int i = 0; i < total_sev; i++)//找出有负载的服务器中负载比较少的，且日费又比较高的，试图把虚拟机从它身上搬出去
		{
...
int main()
{
	...
	for (int i = 0; i < T; i++)//读入一天输出一天
	{
		...
		int left_day = T - i;
		//LL daily_max_core = 0; LL daily_max_memory = 0;
		for (int j = 0; j < R; j++)//把读入的一天的请求都记录在表
		{
			...
			if (strcmp1(opt, "add,") == 1)
			{
				...
				for (int i = 0; i < M; i++)
				{
					...
					{
						//CPU_core_demand += vmachine[i].core;
						//instorage_demand += vmachine[i].memory;
		...
		//迁移
		//本来想对不同闲置率的情况使用不同的迁移策略，没成功
		if (flag_migration == 0 && idle_rate()<0.4&&i>T/3)
		{
			move7();
			if (daily_migration.W > total_vma * 3 / 100)
			{
				flag_migration = 1;
			}
		}
		...
		for (int i = 0; i < daily_opt.top; i++)//一个个处理请求并记录，部署不下就买并记录
		{
			...
			{
				int successfully_deployed;
				if (idle_rate() < 0.4)//本来想对不同闲置率的情况使用不同的部署策略，没成功
				{
				...
				if (successfully_deployed == 0)
				{
					double min_evaluate = 1e17; int min_evaluate_sev_type;
					int need_core = vmachine[daily_opt.vma_type[i]].core;
					int need_memory = vmachine[daily_opt.vma_type[i]].memory;
					if (vmachine[daily_opt.vma_type[i]].isbinode == 0)
					{
						//////在能够装得下这个虚拟机的服务器中，选目前价性比最低的一台，买它
						for (int j = 0; j < N; j++)
						{
							if (sever[j].core / 2 >= need_core&&sever[j].memory / 2 >= need_memory)
							{
								...
								{
									min_evaluate = evaluate_value;
									min_evaluate_sev_type = j;
								}
							}
						}
					}
					else if (vmachine[daily_opt.vma_type[i]].isbinode == 1)
					{
						for (int j = 0; j < N; j++)
						{
